...
```
#Recurrence data compilation TCGA
```{r}
...
complete_subtype = complete_subtype %>% 
    mutate(recurrence_status_binary = ifelse(!is.na(recurrence_site_combo), "yes", "no"))
```
...
complete_subtype <- complete_subtype %>%
  mutate(
    DFS_status = case_when(
      study == "ACRG" & recurrence_site_combo == "Locoregional Recurrence" & DFS_status == 1 ~ 2,
      TRUE ~ DFS_status
    )
  )
```
